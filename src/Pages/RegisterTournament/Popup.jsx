...
const response=await fetch(`${import.meta.env.VITE_SERVER_URL}/api/tournament/registerfortournament`,{
          ...
          <div className="flex min-h-full items-center justify-center p-4 text-center sm:items-center sm:p-0">
            ...
            >
              <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
               ...
               {Individual?
                <>
                 <p className='text-m italic my-4 text-center md:ml-8 md:w-96'>"Life is like playing a violin solo in public and learning the instrument as one goes on.
                "</p>
                <h2 className='text-2xl text-center mt-6 font-semibold'>Individual Registration</h2>
               </>
                :
                <>
                <p className='text-m italic my-4 md:ml-8 md:w-96'>"Collaboration is like carbonation for fresh ideas. Working together bubbles up ideas you would not have come up with solo, which gets you further faster.
                "</p>
                <h2 className='text-2xl text-center mt-3 font-semibold text-red-400'>Team Registration</h2>
                <p className='italic md:w-96 text-start mt-8 '><span className='font-bold'>Note: </span> The person registering will automatically be considered as the team leader.</p>
                </>
            }
               {Individual?
  ...
  <path stroke-linecap="round" stroke-linejoin="round" d="M17.25 8.25L21 12m0 0l-3.75 3.75M21 12H3" />
                  ...
                  {Individual?
                  <p className='text-center mt-2 block md:flex'>Participate as a Team? <span className='text-sky-400 cursor-pointer hover:underline block md:flex' onClick={()=>{setIndividual(false)
                  setTypeReg('team')
                  }}> Click Here</span></p>
                :
                <p className='text-center mt-2 mr-8 block md:flex'>Participate solo? <span className='text-sky-400 cursor-pointer hover:underline block md:flex' onClick={()=>{setIndividual(true)
