...
export function Sidebar() {
 ...
 const [isExpanded, setIsExpanded] = useState(false);
  ...
  return (
    ...
    <aside 
     className={`w-${isExpanded ? '64' : '20'} flex  flex-col overflow-y-auto border-r bg-black px-5 py-8 h-auto transition-width`}
    ...
    >
      ...
      <div className="mt-6 flex flex-1 flex-col justify-between">
          ...
          <Link
            ...
            >
              <BarChart className="h-5 w-5" aria-hidden="true" />
                 {isExpanded && <span className="mx-2 text-sm font-medium">Dashboard</span>}
            </Link>
            ...
            >
              ...
              {isExpanded && <span className="mx-2 text-sm font-medium">Create Tournament</span>}
            </Link>
            :
            <Link
            to='/tournaments/live'
        ...
        >
          ...
          {isExpanded && <span className="mx-2 text-sm font-medium">Participate</span>}
        </Link>
  }
...
{JSON.parse(localStorage.getItem('user_data'))?.role==="Host" && 
            ...
            >
              ...
              {isExpanded && <span className="mx-2 text-sm font-medium">Manage</span>}
            </Link>}
            <Link
                  to='/profile'
              className="flex transform items-center rounded-lg px-3 py-2 text-gray-200 transition-colors duration-300 hover:bg-gray-100 hover:text-gray-700"
            ...
            >
              ...
              {isExpanded && <span className="mx-2 text-sm font-medium">Account Settings</span>}
            </Link>
            <form onSubmit={(e)=>{
              e.preventDefault();
            ...
            >
              ...
              {isExpanded && <span className="mx-2 text-sm font-medium">Log Out</span>}
            </button>
            </form>
          </div>
        </nav>
