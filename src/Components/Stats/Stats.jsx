...
function Stats() {
  ...
  const [liveTournamentCount, setLiveTournamentCount] =
  ...
  const fetchTournamentData = async () => {
    ...
    const filterLiveTournaments = data?.allTournaments?.filter(
      (tournament) =>
        Date.now() >= new Date(tournament.startDateTime) &&
  ...
  return (
    <div className="Stats flex flex-wrap justify-center m-6">
      <Card
        ...
        count={noOfTournaments}
        isLoop={true}
      />
      <Card
        animationData={Live}
        heading={"Live Events"}
        count={liveTournamentCount}
        isLoop={true}
      />
      <Card
